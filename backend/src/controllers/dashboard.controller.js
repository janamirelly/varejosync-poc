...
async function obterDashboard(req, res) {
  ...
  SELECT
    ...
    date('now', 'localtime', '-1 day') AS ontem,
    date('now', 'localtime', '-6 days') AS inicio_7dias
),
resumo_vendas AS (
  SELECT
    COUNT(DISTINCT CASE
      WHEN date(datetime(v.criado_em, 'localtime')) = params.hoje
      THEN v.id_venda
    END) AS pedidos_dia,
    ...
    ROUND(
      COALESCE(SUM(CASE
        WHEN date(datetime(v.criado_em, 'localtime')) = params.ontem
        THEN v.total
        ELSE 0
      END), 0) /
      NULLIF(COUNT(DISTINCT CASE
      ...
      END), 0),
      2
    ) AS ticket_medio_ontem,

    ROUND(MAX(CASE
      WHEN date(datetime(v.criado_em, 'localtime')) = params.hoje
      THEN v.total
    END), 2) AS maior_pedido_dia,

    ROUND(MIN(CASE
      WHEN date(datetime(v.criado_em, 'localtime')) = params.hoje
      THEN v.total
    END), 2) AS menor_pedido_dia
...
`);
    ...
    return res.json({
      cards: {
        ...
        ticket_medio_ontem: Number(cards.ticket_medio_ontem || 0),
        maior_pedido_dia: Number(cards.maior_pedido_dia || 0),
        menor_pedido_dia: Number(cards.menor_pedido_dia || 0),
