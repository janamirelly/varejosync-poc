/* =========================================================
   MIGRATIONS.SQL (limpo)
   - Não recria tabelas base do schema (ex.: venda)
   - Apenas views/relatórios e auditoria
...
-- VIEW (B): vw_dashboard_estoque_abaixo_min_por_produto
...
CREATE TABLE IF NOT EXISTS auditoria (
  ...
  id_usuario   INTEGER,
  acao         TEXT NOT NULL,          -- ex: 'CONSULTAR_ESTOQUE'
  recurso      TEXT NOT NULL,          -- ex: '/estoque', '/produtos/1/estoque'
  detalhes     TEXT,                   -- JSON string com filtros/query/body
  ip           TEXT,
  user_agent   TEXT,
...
-- =========================================================
-- VIEW: Fiscal por período (documento_fiscal já existe no schema)
-- =========================================================
...
-- =========================================================
-- MIGRATION: DESCONTOS / PROMOÇÕES (PDV - Vendedora)
-- ========================================================

-- Backfill: para itens já existentes, define original = preco_unit atual
UPDATE item_venda
SET preco_unit_original = preco_unit
WHERE preco_unit_original IS NULL;


-- Backfill: assume que total atual era "bruto" (antes de descontos)
UPDATE venda
